...
extern "C" int _bit_rev(
    ...
    if (domain_size <= 1024)
        ...
        bit_rev_permutation_z<Z_COUNT><<<dim3(domain_size / Z_COUNT / bsize, poly_count), bsize, bsize * Z_COUNT * sizeof(fr_t)>>>
            (d_out, d_inp, lg_domain_size, padded_poly_size);
    else {
        ...
        bit_rev_permutation_z<Z_COUNT><<<dim3(sm_count * 2, poly_count), 192, 192 * Z_COUNT * sizeof(fr_t)>>>(
            d_out, d_inp, lg_domain_size, padded_poly_size
        );
    }
