 ...
 * Status: MODIFIED from stark-backend/crates/cuda-backend/cuda/src/fri.cu
 * Imported: 2025-09-09 by @felicityin
 * 
 * LOCAL CHANGES (high level): 
 * - 2025-09-09: Replace the original field arithmetic implementation
*/
...
__global__ void cukernel_fri_fold(
    fr4_t *__restrict__ result,
    fr4_t *__restrict__ folded,
    const fr4_t *__restrict__ fri_input,
    fr4_t *__restrict__ d_constants,
    fr_t *__restrict__ g_inv_powers,
    uint64_t N
) {
    fr4_t half_beta = d_constants[0];   // beta/2
    fr4_t half_one = d_constants[1];    // 1/2
    fr4_t beta_square = d_constants[2]; // beta^2
    uint64_t idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (idx < N) {
        fr4_t beta_g_inv = half_beta * g_inv_powers[idx]; // beta/2 * g_inv^i
        fr4_t c1 = half_one + beta_g_inv;                 // 1/2 + beta/2 * g_inv^i
        fr4_t c2 = half_one - beta_g_inv;                 // 1/2 - beta/2 * g_inv^i
        fr4_t a = folded[2 * idx];
        fr4_t b = folded[2 * idx + 1];
        fr4_t res = c1 * a;
        res += c2 * b;
...
__global__ void compute_diffs(
    fr4_t *__restrict__ diffs,
    fr4_t *__restrict__ d_z,
    fr_t *__restrict__ d_domain,
    uint32_t log_n
) {
    ...
    uint32_t N = 1 << log_n;
    fr4_t z = *d_z;
    fr_t shift = d_domain[0];
    fr_t g = d_domain[1];
    fr_t g_idx = g^idx;
    fr_t g_pow = g^stride;

    for (; idx < N; idx += stride, g_idx *= g_pow) {
        fr4_t diff = z - shift * g_idx;
        diffs[idx] = diff;
    }
}

// data[i] = g^i for i in 0..N
__global__ void powers(fr_t *__restrict__ data, fr_t *__restrict__ d_g, uint32_t N) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t stride = blockDim.x * gridDim.x;
    fr_t g = *d_g;
    fr_t g_idx = g^idx;
...
__global__ void powers_ext(fr4_t *__restrict__ data, fr4_t *__restrict__ d_g, uint32_t N) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t stride = blockDim.x * gridDim.x;
    fr4_t g = *d_g;
    fr4_t g_idx = g^idx;
    fr4_t g_pow = g^stride;
...
__global__ void fpext_bit_reverse(fr4_t *data, uint32_t log_n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t stride = blockDim.x * gridDim.x;
    uint32_t N = 1 << log_n;
...
// Batch inversion
__global__ void batch_invert(fr4_t *data, uint64_t log_n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t stride = blockDim.x * gridDim.x;
    ...
    j -= 1;
    pos -= stride;
    // accum_inv = inv(prod(data[idx], data[idx+stride], ..., data[idx+(j-1)*stride]))
    fr4_t accum_inv = accums[j].reciprocal();

    for (; j > 0; pos -= stride, j -= 1) {
        fr4_t tmp = accum_inv * accums[j - 1]; // inv(data[pos])
        accum_inv *= data[pos];
        data[pos] = tmp;
    }
...
__global__ void reduce_matrix_quotient_acc(
    fr4_t *__restrict__ quotient_acc,
    fr_t *__restrict__ matrix,
    fr4_t *__restrict__ z_diff_invs,
    const fr4_t *__restrict__ matrix_eval,
    fr4_t *__restrict__ d_alphas,
    fr4_t *__restrict__ d_alphas_offset,
    uint32_t width,
    uint32_t height,
    bool is_first
) {
    ...
    fr4_t mz = *matrix_eval;
    fr4_t alpha_offset = *d_alphas_offset; // alpha^matrix_offset
    fr4_t quotient = alpha_offset * z_diff_invs[row_idx] * (mz - accum);
    if (is_first) {
...
__global__ void cukernel_split_ext_poly_to_base_col_major_matrix(
    ...
    for (uint32_t col_idx = 0; col_idx < col_num; col_idx++) {
        fr4_t ext_val = d_poly[row_idx * col_num + col_idx];
        d_matrix[(col_idx * 4 + 0) * matrix_height + br_row_idx] = ext_val[0];
        d_matrix[(col_idx * 4 + 1) * matrix_height + br_row_idx] = ext_val[1];
        d_matrix[(col_idx * 4 + 2) * matrix_height + br_row_idx] = ext_val[2];
        d_matrix[(col_idx * 4 + 3) * matrix_height + br_row_idx] = ext_val[3];
    }
}
...
__global__ void matrix_evaluate_chunked(
    fr4_t *__restrict__ partial_sums,
    const fr_t *__restrict__ matrix,
    const fr4_t *__restrict__ inv_denoms,
    fr_t g,
    uint32_t height,
    uint32_t width,
    uint32_t chunk_size,
    ...
    __shared__ fr4_t sdata[FRI_MAX_THREADS];
#else
    __shared__ __align__(alignof(fr4_t)) unsigned char s_data_int[FRI_MAX_THREADS * sizeof(fr4_t)];
    fr4_t *sdata = reinterpret_cast<fr4_t *>(s_data_int);
#endif

    fr4_t thread_sum = {0, 0, 0, 0};
    ...
    if constexpr (INV_DENOMS_BITREV) {
        g_power = g^bit_rev(chunk_start + tid, height);
    } else {
        g_power = g^(chunk_start + tid);
    }

    fr_t g_stride = g^blockDim.x;
    for (uint32_t i = tid; i < chunk_range; i += blockDim.x) {
        ...
        if constexpr (INV_DENOMS_BITREV) {
            ...
            if (i + blockDim.x < chunk_range) {
                g_power = g^bit_rev(domain_idx + blockDim.x, height);
            }
        } else {
        ...
        thread_sum += fr4_t(fr_t(raw_val)) * weight;
    }
...
__global__ void matrix_evaluate_finalize(
    fr4_t *__restrict__ output,
    const fr4_t *__restrict__ partial_sums,
    fr4_t scale_factor,
    uint32_t num_chunks,
    uint32_t width
) {
...
extern "C" int _compute_diffs(fr4_t *diffs, fr4_t *d_z, fr_t *d_domain, uint32_t log_max_height) {
...
extern "C" int _fpext_bit_reverse(fr4_t *diffs, uint32_t log_max_height) {
    auto block = FRI_MAX_THREADS;
    auto grid = get_num_sms() * 2;
...
extern "C" int _batch_invert(fr4_t *diffs, uint32_t log_max_height, uint32_t invert_task_num) {
    auto [grid, block] = kernel_launch_params(invert_task_num, FRI_MAX_THREADS);
    batch_invert<<<grid, block>>>(diffs, log_max_height);
    return cudaGetLastError();
}

extern "C" int _powers(fr_t *data, fr_t *g, uint32_t N) {
    auto block = FRI_MAX_THREADS;
    auto grid = get_num_sms() * 2;
...
extern "C" int _powers_ext(fr4_t *data, fr4_t *g, uint32_t N) {
    auto block = FRI_MAX_THREADS;
    auto grid = get_num_sms() * 2;
...
extern "C" int _reduce_matrix_quotient_acc(
    ...
    fr_t *matrix,
    fr4_t *z_diff_invs,
    const fr4_t *matrix_eval,
    fr4_t *d_alphas,
    fr4_t *d_alphas_offset,
    uint32_t width,
    uint32_t height,
    bool is_first
) {
