 ...
 * Status: MODIFIED from stark-backend/crates/cuda-backend/cuda/src/poseidon2.cu
 * Imported: 2025-09-08 by @felicityin
 * 
 * LOCAL CHANGES (high level):
 * - 2025-09-08: Replace the original field arithmetic implementation
*/
...
__global__ void poseidon2_rows_p3_multi_kernel(
    ...
    for (uint m = 0; m < matrix_num; m++) {
        ...
        fr_t *matrix = (fr_t *)(matrices_ptr[m]);
        for (uint i = 0; i < col_size; i++) {
            cells[used++] = matrix[i * row_size + gid];
            if (used == CELLS_RATE) {
                poseidon2::poseidon2_permut(cells);
                used = 0;
            }
        }
    }
...
__global__ void cukernel_query_digest_layers(
    fr_t *d_digest_matrix,        // fr_t*, also Digest: CELLS_OUT=8 fr_t elements
    ...
    uint64_t num_layer            // e.g. 23
) {
    const uint32_t ELEM_PER_DIGEST = CELLS_OUT; // 8 * fr_t
    ...
    fr_t *d_layer = (fr_t *)d_layers_ptr[layer_idx];
    uint64_t digest_offset = d_indices[query_idx * num_layer + layer_idx];
    fr_t digest_elem = d_layer[digest_offset * ELEM_PER_DIGEST + elem_offset];
