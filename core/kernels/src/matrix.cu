 ...
 * Status: MODIFIED from stark-backend/crates/cuda-backend/cuda/src/matrix.cu
 * Imported: 2025-09-06 by @felicityin
 * 
 * LOCAL CHANGES (high level):
 * - 2025-09-05: Replace the original field arithmetic implementation
*/
...
template __global__ void cukernel_matrix_transpose<fr_t>(fr_t *, const fr_t *, size_t, size_t);
template __global__ void cukernel_matrix_transpose<fr4_t>(fr4_t *, const fr4_t *, size_t, size_t);
...
__global__ void cukernel_split_ext_poly_to_multiple_base_matrix(
    ...
    fr_t *d_matrix = (fr_t *)d_matrix_ptr[chunk_idx];
    // assumption:
    //   1. fr4_t::D = 4, the input fr4_t will be flatten to Fp
    //   2. the order of the matrix is column major
    // uint64_t matrix_width = 4; // fr4_t::D
    ...
    fr4_t ext_val = d_poly[row_idx];
    d_matrix[matrix_height * 0 + chunk_row] = ext_val[0];
    d_matrix[matrix_height * 1 + chunk_row] = ext_val[1];
    d_matrix[matrix_height * 2 + chunk_row] = ext_val[2];
    d_matrix[matrix_height * 3 + chunk_row] = ext_val[3];
}
...
extern "C" int matrix_transpose_fp(fr_t *output, const fr_t *input, size_t col_size, size_t row_size) {
