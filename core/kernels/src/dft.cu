 ...
 * Status: MODIFIED from stark-backend/crates/cuda-backend/cuda/src/lde.cu
 * Imported: 2025-09-07 by @felicityin
 * 
 * LOCAL CHANGES (high level): 
 * - 2025-09-06: Replace the original field arithmetic implementation
*/
...
__global__ void multi_bit_reverse_kernel(fr_t *io, const uint32_t nBits, const uint32_t count) {
...
extern "C" int _multi_bit_reverse(fr_t *io, const uint32_t nBits, const uint32_t count) {
