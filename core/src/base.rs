...
use cuda_common::{copy::MemCopyD2H, d_buffer::DeviceBuffer, error::MemCopyError};
...
impl<T> Clone for DeviceMatrix<T> {
    fn clone(&self) -> Self {
        Self { buffer: Arc::clone(&self.buffer), height: self.height, width: self.width }
    }
}
...
impl<T> DeviceMatrix<T> {
    pub fn new(buffer: Arc<DeviceBuffer<T>>, height: usize, width: usize) -> Self {
        assert_ne!(height * width, 0, "Zero dimensions h {height} w {width} are wrong");
        assert_eq!(buffer.len(), height * width, "Buffer size must match dimensions");
        Self { buffer, height, width }
    }

    pub fn with_capacity(height: usize, width: usize) -> Self {
        Self { buffer: Arc::new(DeviceBuffer::with_capacity(height * width)), height, width }
    }

    pub fn dummy() -> Self {
        Self { buffer: Arc::new(DeviceBuffer::new()), height: 0, width: 0 }
    }
...
impl<T, B> DevicePoly<T, B> {
    pub fn new(is_bit_reversed: bool, coeff: DeviceBuffer<T>) -> Self {
        Self { is_bit_reversed, coeff, _marker: PhantomData }
    }
