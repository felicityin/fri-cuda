...
use cuda_common::d_buffer::DeviceBuffer;

use crate::{cuda::ntt, types::F};

const MAX_LG_DOMAIN_SIZE: usize = 24;
...
impl NttParameters {
    fn new(inverse: bool) -> Self {
        ...
        Self { twiddles, partial_twiddles }
    }
}
...
impl<'a> Ntt<'a> {
    fn new(
        ...
        let ntt_parameters = if is_intt { inverse_params() } else { forward_params() };
        Self {
            buffer,
            lg_domain_size,
            padded_poly_size,
...
pub(super) fn batch_ntt(
    ...
    let mut ntt = Ntt::new(buffer, log_trace_height, padded_poly_size, width, is_intt);

    if log_trace_height <= 10 {
        ntt.step(log_trace_height);
    } else if log_trace_height <= 17 {
        let step = log_trace_height / 2;
        ntt.step(step + log_trace_height % 2);
        ntt.step(step);
    } else if log_trace_height <= 30 {
        let step = log_trace_height / 3;
        let rem = log_trace_height % 3;
        ntt.step(step);
        ntt.step(step + (if log_trace_height == 29 { 1 } else { 0 }));
        ntt.step(step + (if log_trace_height == 29 { 1 } else { rem }));
    } else if log_trace_height <= 40 {
        let step = log_trace_height / 4;
        let rem = log_trace_height % 4;
        ntt.step(step);
        ntt.step(step + (if rem > 2 { 1 } else { 0 }));
        ntt.step(step + (if rem > 1 { 1 } else { 0 }));
        ntt.step(step + (if rem > 0 { 1 } else { 0 }));
    } else {
