use cuda_common::copy::MemCopyH2D;
...
use crate::{
    ...
    cuda::kernels::{dft::*, matrix::matrix_get_rows_fp_kernel},
    types::F,
};
...
pub(super) fn compute_lde_matrix(
    ...
    unsafe {
        ...
        batch_ntt(lde_matrix.buffer(), log_trace_height, log_blowup, width as u32, true, true);

        if shift != F::ONE {
            zk_shift(lde_matrix.buffer(), lde_size, log_lde_height, shift).unwrap();
        }

        batch_bit_reverse(lde_matrix.buffer(), log_lde_height, lde_size).unwrap();

        batch_ntt(lde_matrix.buffer(), log_lde_height, 0, width as u32, false, false);
    }
