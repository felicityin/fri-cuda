...
use crate::{
    base::{DeviceMatrix, DevicePoly, ExtendedLagrangeCoeff, MatrixDimensions},
    cuda::kernels::{dft::batch_bit_reverse, fri::*},
    types::*,
};
...
// Rreturn a map from point to inverse denominators of different heights in non-bitrev order
...
// for one point is 2^23 * 2 * sizeof<EF>. for field like KoalaBear and its deg 4 extension, the
// memory usage is 2^23 * 2 * 16 = 256MB.
...
pub(crate) fn fri_fold(
    ...
    // We don't support folded poly whose length exceed 2^24
    // as we take advantage of the fact g_inv is a base field element if folded.len() <= 2^24
    assert!(log2_strict_usize(folded.len()) <= 24);
    assert!(g_inv.as_base_slice()[1..].iter().all(F::is_zero)); // g_inv.is_in_basefield()
    assert!(folded.is_bit_reversed);

    let half_one = F::ONE.halve().into();
    ...
    unsafe {
        ...
        batch_bit_reverse(&g_invs, log2_ceil_usize(half_folded_len) as u32, half_folded_len as u32)
            .unwrap();
        fri_fold_kernel(
            &d_result,
            &folded.coeff,
