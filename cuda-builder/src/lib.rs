...
impl CudaBuilder {
    ...
    pub fn watch<P: AsRef<Path>>(mut self, path: P) -> Self {
        self.watch_paths.push(path.as_ref().to_string_lossy().to_string());
        self
    }
    ...
    pub fn link_search<P: AsRef<Path>>(mut self, path: P) -> Self {
        self.link_search_paths.push(path.as_ref().to_string_lossy().to_string());
        self
    }
    ...
    pub fn build(self) {
        ...
        if let Ok(cuda_path) = env::var("CUDA_PATH") {
            builder.include(format!("{cuda_path}/include"));
        }
        ...
        builder.flag("-gencode").flag(format!("arch=compute_{cuda_arch},code=sm_{cuda_arch}"));
        ...
        if cuda_opt_level == "0" {
            ...
            builder.debug(false).flag(format!("--ptxas-options=-O{cuda_opt_level}"));
        }
    ...
    fn validate(&self) {
        ...
        if self.source_files.is_empty() {
            panic!(
                "At least one source file must be added using .file() or .files() before calling .build()"
            );
        }
    ...
    fn handle_debug_shortcuts(&self) {
        if env::var("CUDA_DEBUG").map(|v| v == "1").unwrap_or(false) {
            unsafe { env::set_var("CUDA_OPT_LEVEL", "0") };
            unsafe { env::set_var("CUDA_LAUNCH_BLOCKING", "1") };
            unsafe { env::set_var("CUDA_MEMCHECK", "1") };
            unsafe { env::set_var("RUST_BACKTRACE", "full") };
            println!(
                "cargo:warning=CUDA_DEBUG=1 → forcing CUDA_OPT_LEVEL=0, CUDA_LAUNCH_BLOCKING=1, CUDA_MEMCHECK=1, RUST_BACKTRACE=full"
            );
        }
    }
}
...
pub fn detect_cuda_arch() -> String {
    ...
    unsafe { env::set_var("CUDA_ARCH", &arch) };
    arch
}
...
pub fn nvcc_parallel_jobs() -> String {
    ...
    let threads =
        env::var("NVCC_THREADS").ok().and_then(|v| v.parse::<usize>().ok()).unwrap_or(threads);
