...
impl CudaError {
    ...
    pub fn new(code: i32) -> Self {
        CudaError { code, name: get_cuda_error_name(code), message: get_cuda_error_string(code) }
    }

    /// Returns `Ok(())` if `code == 0` (cudaSuccess), or `Err(CudaError)` if non-zero.
    pub fn from_result(code: i32) -> Result<(), Self> {
        if code == 0 { Ok(()) } else { Err(Self::new(code)) }
    }
}
...
pub enum MemCopyError {
    ...
    SizeMismatch { operation: &'static str, host_len: usize, device_len: usize },
}
