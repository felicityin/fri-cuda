...
impl<T> DeviceBuffer<T> {
    ...
    pub fn new() -> Self {
        DeviceBuffer { ptr: ptr::null_mut(), len: 0 }
    }
    ...
    pub fn with_capacity(len: usize) -> Self {
        ...
        DeviceBuffer { ptr: typed_ptr, len }
    }
    ...
    pub fn fill_zero_suffix(&self, start_idx: usize) -> Result<(), CudaError> {
        assert!(start_idx < self.len, "start index has to be smaller than length");
    ...
    pub fn view(&self) -> DeviceBufferView {
        DeviceBufferView { ptr: self.ptr as *const c_void, size: self.len * size_of::<T>() }
    }
}
