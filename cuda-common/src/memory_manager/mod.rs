...
use crate::{
    error::{MemoryError, check},
    stream::{cudaStream_t, cudaStreamPerThread},
};
...
impl MemoryManager {
    pub fn new() -> Self {
        ...
        Self { pool, allocated_ptrs: HashMap::new(), current_size: 0, max_used_size: 0 }
    }
    ...
    pub fn d_malloc(&mut self, size: usize) -> Result<*mut c_void, MemoryError> {
        ...
        let ptr = if size < self.pool.page_size {
            ...
            self.allocated_ptrs.insert(NonNull::new(ptr).expect("cudaMalloc returned null"), size);
            Ok(ptr)
        } else {
...
impl MemTracker {
    pub fn start(label: &'static str) -> Self {
        let current =
            MEMORY_MANAGER.get().and_then(|m| m.lock().ok()).map(|m| m.current_size).unwrap_or(0);
    ...
    pub fn tracing_info(&self, msg: impl Into<Option<&'static str>>) {
        ...
        tracing::info!(
            ...
            msg.into().map_or(self.label.to_string(), |m| format!("{}:{}", self.label, m))
        );
    }
