...
impl VirtualMemoryPool {
                ...
                Ok(_) => {
                    ...
                    let page_size = match std::env::var("VPMM_PAGE_SIZE") {
                        Ok(val) => {
                            ...
                            assert!(
                                custom_size > 0 && custom_size.is_multiple_of(gran),
                                "VPMM_PAGE_SIZE must be > 0 and multiple of {gran}"
                            );
                            custom_size
                        }
                        Err(_) => gran,
    ...
    fn create_new_pages(&mut self, memory_size: usize) -> Result<(), MemoryError> {
        ...
        for i in 0..pages {
            unsafe {
                ...
                self.active_pages.insert(new_base + (i * self.page_size) as u64, handle);
            }
        }
    ...
    pub(super) fn malloc_internal(&mut self, requested: usize) -> Result<*mut c_void, MemoryError> {
        ...
        if let Some(ptr) = best_region {
            ...
            if size > requested {
                self.free_regions.insert(ptr + requested as u64, size - requested);
            }
    ...
    pub(super) fn free_internal(&mut self, ptr: *mut c_void) -> Result<usize, MemoryError> {
        ...
        let size = self.used_regions.remove(&ptr).ok_or(MemoryError::InvalidPointer)?;
    ...
    fn defragment_or_create_new_pages(
        ...
        if to_defrag.last().is_some_and(|(p, s)| p + *s as u64 == self.curr_end) {
        ...
        for (ptr, size) in to_defrag {
            ...
            for i in 0..size / self.page_size {
                ...
                let handle = self.active_pages.remove(&page).ok_or(MemoryError::InvalidPointer)?;
