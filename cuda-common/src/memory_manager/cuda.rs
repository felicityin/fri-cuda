...
pub(super) unsafe fn vpmm_check_support(device_ordinal: i32) -> Result<bool, CudaError> {
    let status = unsafe { _vpmm_check_support(device_ordinal) };
    if status == 0 { Ok(true) } else { Err(CudaError::new(status)) }
}

pub(super) unsafe fn vpmm_min_granularity(device_ordinal: i32) -> Result<usize, CudaError> {
    let mut granularity: usize = 0;
    CudaError::from_result(unsafe { _vpmm_min_granularity(device_ordinal, &mut granularity) })?;
    Ok(granularity)
}

pub(super) unsafe fn vpmm_reserve(size: usize, align: usize) -> Result<CUdeviceptr, CudaError> {
    let mut va_base: CUdeviceptr = 0;
    CudaError::from_result(unsafe { _vpmm_reserve(size, align, &mut va_base) })?;
    Ok(va_base)
}

pub(super) unsafe fn vpmm_release_va(base: CUdeviceptr, size: usize) -> Result<(), CudaError> {
    CudaError::from_result(unsafe { _vpmm_release_va(base, size) })
}
...
pub(super) unsafe fn vpmm_create_physical(
    ...
    CudaError::from_result(unsafe { _vpmm_create_physical(device_ordinal, bytes, &mut h) })?;
    Ok(h)
}
...
pub(super) unsafe fn vpmm_map_and_set_access(
    ...
    CudaError::from_result(unsafe { _vpmm_map_and_set_access(va, bytes, h, device_ordinal) })
}
...
pub(super) unsafe fn vpmm_unmap_release(
    ...
    CudaError::from_result(unsafe { _vpmm_unmap_release(va, bytes, h) })
}
